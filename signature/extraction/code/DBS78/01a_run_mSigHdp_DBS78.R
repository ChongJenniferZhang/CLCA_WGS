...
input_file <-
  file.path(project_dir, "signature/extraction/input/DBS78/clca_catalog_DBS78.csv")
dbs_catalog <- ICAMS::ReadCatalog(file = input_file)

output_dir <- file.path(project_dir, "signature/extraction/raw_output/DBS78/mSigHdp/")
...
time_used <- system.time(expr = {
  mSigHdp::RunHdpxParallel(input.catalog = dbs_catalog, 
                           ground.truth.sig  = cosmicsig::COSMIC_v3.2$signature$GRCh37$DBS78, #use COSMIC signatures as ground truth signatures
                           ...
                           K.guess            = 5, #estimated initiating number of signatures
