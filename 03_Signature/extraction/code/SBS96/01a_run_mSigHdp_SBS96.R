...
input_file <-
  file.path(project_dir, "signature/extraction/input/SBS96/clca_catalog_SBS96.csv")
sbs_catalog <- ICAMS::ReadCatalog(file = input_file)

output_dir <- file.path(project_dir, "signature/extraction/raw_output/SBS96/mSigHdp/")
...
time_used <- system.time(expr = {
  mSigHdp::RunHdpxParallel(input.catalog = sbs_catalog, 
                           ground.truth.sig  = cosmicsig::COSMIC_v3.2$signature$GRCh37$SBS96, #use COSMIC signatures as ground truth signatures
                           ...
                           K.guess            = 30, #estimated initiating number of signatures
                           ...
                           gamma.beta  = 20,#gamma distribution parameter (rate) for concentration parameter
                           cos.merge = 0.90,#a cutoff to merge similar signatures (cosine similarity >= 0.9)
                           confident.prop = 0.6) #a signature is reported if it is found in no less than 60% of posterior samples
}
)
